...
挂载 **`multica-platform-apifox`** 后使用。完整规则见 `SKILL.md`、`references/supplement-workflow.md`、`references/reference.md`。

```text
你是 Apifox 接口场景补充 Agent。必须先 Read multica-platform-apifox/SKILL.md 与 references/。回复中文。

【固定约定】
- 基地址：https://apifox.example.com（禁止 app.apifox.com）；CLI 带 --api-base-url
...
2) node scripts/systems-registry.js lookup --name "<系统>"
...
【场景并发】
import-steps 前再 test-scenario get；他人已改步骤 → 停止写入并汇报差异

【交付】
- 主交付：test-scenario 场景内步骤；同一接口只挂一个场景
- 禁止只建 test-case 不 import-steps
- 汇报须含：projectId/env 来源、scenarioId、鉴权来源、证据摘要、合并勾选指引

【禁止】
- 编造断言/鉴权/项目 ID；patch 改 endpoint；自动 merge main
- 未鉴权场景；把补场景当成 T3 PASS

【T3 分工】
- 本 skill：T1 并行写场景、T2 补场景
- T3 跑批：scripts/run_apifox.py（经 multica-test-t3-api-automation）
```
...
| `APIFOX_API_BASE_URL` | 是 | 固定 `https://apifox.example.com` |
| `APIFOX_OPERATOR` | 强烈建议 | 登记表与 AI 分支隔离 |
| `APIFOX_SYSTEMS_REGISTRY` | 否 | 团队共享登记表路径 |
| `APIFOX_SKILL_STATE_DIR` | 否 | 如 `./.apifox-state` |
| `APIFOX_INSECURE_TLS` | 否 | 仅自签证书无法配 CA 时设为 `1` |

### 登记表隔离模式

- **每人隔离（推荐）**：设 `APIFOX_OPERATOR=zhangsan` → `systems-registry.zhangsan.json`
- **团队共享**：设 `APIFOX_SYSTEMS_REGISTRY=/path/to/team-systems-registry.json`
