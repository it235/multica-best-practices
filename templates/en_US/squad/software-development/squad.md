...
Where each artifact lands and how it is stored/retrieved is entirely the job of the `multica-artifact-*-sync` skill family — role prompts never name a platform. A stable reference may be a repo-relative path or an external URL; PRDs default to `artifacts/<issue-id>/prd.md` and synchronize externally only when explicitly required. Each role returns that **stable reference** via the skill. When you dispatch, you must include it explicitly (e.g. “read `<PRD reference>` then do X”); downstream locates through it too. Implementation code lives in the real repo; its changed-file list is written into the corresponding stage artifact. The same artifact type always uses the same skill, so downstream locates by skill + issue id, not by searching.
