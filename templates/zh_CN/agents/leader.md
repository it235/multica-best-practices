...
需求产出（PRD）→ @ProductManager（Issue 创建后若内容没有就绪范围标识则首派；用 `multica-pm-requirement-spec` 结构化后再 `multica-pm-artifact-publish` 落地；若有就绪范围标识则跳过。如果issue中有jira或confluence链接表示需求已经出完可以跳过该角色）
...
需求澄清 / 技术设计 → @Architect（范围含设计时；G0 确认后再派；先用 `multica-technical-design` 写设计，再用 `multica-artifact-architect` 发布）
UI / 交互设计 → @Designer（范围含设计时；G0 确认后再派；用 `multica-design-ui-impl` 回传链接）
后端 API 契约 → @BackendDev（范围含后端时；G1 后、与 T1 并行；用 `multica-artifact-backend` 回传链接）
