...
description: 产品经理需求规范：把 Issue/诉求结构化为带编号 PRD（G-/FR-/BR-/AC-/KPI-/OP-/RISK-）。含知识库背景检索；结构就绪后交 multica-pm-artifact-publish 落地。
metadata:
  orchestrates:
    - 团队知识库（可选）
  origin:
    - multica-pm-requirement-spec
version: 2.0.0
---
...
> 与 `multica-pm-artifact-publish` 分工：**本 skill = 需求规范与编号；publish = Confluence / JIRA 创建与更新**。换平台只改 publish，不动本 skill。
...
| `multica-platform-jira` / `multica-platform-confluence` | **不直接调用**；读写由 `multica-pm-artifact-publish` 编排 |
...
| **B 提交** | 用户/Leader 确认范围后 → `multica-pm-artifact-publish` | 回传 Confluence + JIRA 链接 |
...
| Review 结论 | PM 动作 |
| --- | --- |
| **FAIL** | 按修改清单改 PRD 编号条目 → 更新修订记录 → `multica-pm-artifact-publish` **Workflow B 更新** → 请 ProductReviewer 复审 |
...
```text
1. multica-pm-requirement-spec   — 结构化 PRD（本 skill）
2. multica-pm-artifact-publish — Confluence + JIRA 创建/更新 + 可选钉钉
```
