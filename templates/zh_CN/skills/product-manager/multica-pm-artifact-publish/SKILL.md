...
> 内容结构由 `multica-pm-requirement-spec` 负责；本 skill 只管「提交到哪里、如何更新」。
...
```text
1. multica-pm-requirement-spec  — 结构化 PRD + 修订记录
2. multica-pm-artifact-publish — 本 skill：Confluence + JIRA + 可选钉钉
```
...
```text
先用 multica-pm-requirement-spec 结构化 PRD，
再用 multica-pm-artifact-publish 落地并回传链接；
