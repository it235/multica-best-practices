...
0. 需求产出（范围含 @ProductManager）→ @ProductManager 用 `multica-artifact-req-sync` 出 PRD（含 OP- 待确认清单）并回传链接
...
2. 设计（范围含设计）→ @Architect 用 `multica-artifact-design-sync` 出设计并回传引用
